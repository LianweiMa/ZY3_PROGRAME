...
#ifndef _RemoveDir
#define _RemoveDir
inline BOOL RemoveDir(LPCSTR szPath){ char strCmd[1024]; sprintf( strCmd,"/bin/rm -f -r \"%s\"",szPath ); system( strCmd ); }
#endif

#ifndef _CopyFile
#define _CopyFile
inline BOOL CopyFile( LPCSTR lpExistingFileName, LPCSTR lpNewFileName, BOOL bFailIfExists)
{ if( !strcmp(lpExistingFileName,lpNewFileName) ) return TRUE; if(IsExist(lpNewFileName)) { if(bFailIfExists) return FALSE; else remove(lpNewFileName);	} char strCmd[1024]; sprintf( strCmd,"/bin/cp -f \"%s\" \"%s\"",lpExistingFileName,lpNewFileName ); system( strCmd ); return TRUE;}
#endif

inline BOOL CreateLink(LPCSTR szTarget,LPCSTR szLinkName)
{
	char strCmd[1024]; 
	sprintf(strCmd,"/bin/ln -s \"%s\" \"%s\" ",szTarget,szLinkName);
	if ( system(strCmd) == 0 ){
		return FALSE;
	}
	return TRUE;
...
inline void RemoveLink(LPCSTR szLinkName)
{
	char strCmd[1024]; sprintf( strCmd,"/bin/unlink \"%s\"",szLinkName ); system( strCmd );
}
